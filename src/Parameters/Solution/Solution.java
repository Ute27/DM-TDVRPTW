...
public class Solution {
    ...
    private final double infeasibleRegret = -1000;
    private final double infeasibleCost = 999999999;
    private Random rnd;
    ...
    public Solution(Solution sol) {
        ...
        this.seedNumber = sol.getSeedNumber();
    ...
    public Solution(List<List<Customer>> customers, ProblemInstance problem, long seedNumber) {
        this.tdi = problem.getTdi();
    ...
    public void repairRegretRANDOPT(Customer customerToInsert) {
        ...
        for(SingleTrip route: routes) {
            double[][] mic = calculateMIC_Random(route, customerToInsert, problem);
    ...
    public void repairRegretRANDRAND(Customer customerToInsert) {
        int k_index = rnd.nextInt(3); //index equals 0,1 or 2.
        int k = k_index+1;
        //col1 = route index
        //col2 = regret value
    ...
    public void repairRegretOPTOPT(Customer customerToInsert) {
        int k_index = rnd.nextInt(3); //index equals 0,1 or 2.
        int k = k_index+1;
        //col1 = route index
        //col2 = regret value
    ...
    public void repairRegretEARLYOPT(Customer customerToInsert) {
    ...
    public void repairRegretEARLYEARLY(Customer customerToInsert) {
    ...
    public double[][] calculateMIC_Early(SingleTrip route, Customer customerToInsert, ProblemInstance problem) {
        ...
        Arrays.stream(result).forEach(a -> Arrays.fill(a, infeasibleCost));
        ...
        for(int index_insertionPlace = 1; index_insertionPlace<customers.size()-1; index_insertionPlace++) {
            ...
            double adjustedCost = totalCost - route.getDuration();
            simulateTrip(problem, result, index_insertionPlace, customerCopy, optimalDT, adjustedCost);
    ...
    public double[][] calculateMIC_Random(SingleTrip route, Customer customer, ProblemInstance problem) {
        ...
        for(int index_insertionPlace = 1; index_insertionPlace<customers.size(); index_insertionPlace++) {
            ...
            double randomDT = earlyDT + (lateDT - earlyDT) * rnd.nextDouble() ;
            double adjustedCost = totalCost - route.getDuration();
            simulateTrip(problem, result, index_insertionPlace, customerCopy, randomDT, adjustedCost);
    ...
    private void simulateTrip(ProblemInstance problem, double[][] result, int index_insertionPlace, List<Customer> customerCopy, double randomDT, double adjustedCost) {
        SingleTrip st = new SingleTrip(customerCopy,randomDT,problem);
        if(st.getFeasibility() && st.getDuration()<infeasibleCost) {
            result[index_insertionPlace][0] = adjustedCost + st.getDuration();
            result[index_insertionPlace][1] = index_insertionPlace-1;
            result[index_insertionPlace][2] = randomDT;
        } else {
            result[index_insertionPlace][0] = infeasibleCost;
            result[index_insertionPlace][1] = index_insertionPlace-1;
        }
    }
    ...
    private void insertCustomerBasedOnMICValues(Customer customerToInsert, double[][] mic_regretValues) {
        Arrays.sort(mic_regretValues, Comparator.comparingDouble(row -> -row[3]));
